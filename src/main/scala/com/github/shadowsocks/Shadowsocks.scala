...
class Shadowsocks extends AppCompatActivity with ServiceBoundContext {
      ...
      handler.post(() => {
        s match {
          case State.CONNECTING =>
            ...
            //将该ic_start_busy图片替换成ic_stat_shadowsocks
            fab.setImageResource(R.drawable.ic_stat_shadowsocks)
  ...
  private def changeSwitch(checked: Boolean) {
    ...
    //ic_start_connected 替换成 ic_launcher    ic_start_idle替换成 ic_launcher
    fab.setImageResource(if (checked) R.drawable.ic_launcher else R.drawable.ic_launcher)
    if (fab.isEnabled) {
      fab.setEnabled(false)
  ...
  override def onCreate(savedInstanceState: Bundle) {
    ...
    toolbar.setTitle("氢度") // non-translatable logo
